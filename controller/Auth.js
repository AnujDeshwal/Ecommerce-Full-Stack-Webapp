...
const { otp_generator } = require("../services/otp_generator");
...
exports.logout = async (req, res) => {
  // we are setting the user cookie here for 1 hr with jwt token
  const user = req.user;
  //   here we are basically setting the user cookie to be null and here duration is zero because as you see above we have not extended the Date.now() which means as soon as it is created it will be finished
  res
    .cookie("jwt", null, {
...
exports.otpGenerate = async (req, res) => {
  try {
    // watch if await can be used , we will get the email from the request
    const user = await User.findOne({ email: req.body.email });
    if (user) {
      res.sendStatus(401);
    } else {
      ...
      // below always use single quotes inside the double quotes
      const html = `<p>This is your one time password : ${OTP} to verify your email address </p>`;

      if (req.body.email) {
        console.log("hello mrs");
        const response = await sendMail({ to: req.body.email, subject, html });
        //   here i am sending the otp to the frontend so that whatever user will enter the otp in the frontend so i will cross check with it and if it is correct then i will dispatch the createUser thunk function
        res.json({ OTP });
      } else {
        // exist nahi karta
    ...
    console.log("again in the catch");
    res.sendStatus(401);
  }
};
